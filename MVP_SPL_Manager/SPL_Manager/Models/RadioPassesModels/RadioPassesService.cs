...
namespace SPL_Manager.Models.RadioPassesModels
{
    ...
    internal class RadioPassesService : IRadioPassesService
    {
        public async Task<List<pass>> GetNextPasses(int satId = -1)
        {
            ...
            int noradID = -1;
            dynamic SatSettings = null;


            SatSettings = ((IEnumerable<dynamic>)ProgramProps.settings.satInfo).ToList().Find(x => x.id == satId);
            if (SatSettings.noradID == -1) return emptyResult;
            noradID = (int)SatSettings.noradID;
