using Nest;
using SPL_Manager.Models.General;
...
namespace SPL_Manager.Models.SatPacketModels
{
    public class PacketObject
    {
        ...
        public int GetTypeIndex() => JsonObject.Types.FindIndex(item => item.Id == Type);
        public int GetSubtypeIndex() => JsonObject.Types[GetTypeIndex()].SubTypes.FindIndex(item => item.Id == Subtype);
        public string GetTypeName() => Type == -1 ? "Error" : JsonObject.Types[GetTypeIndex()]?.Name;
        public string GetSubTypeName() => Type == -1 ? "logs" : JsonObject.Types[GetTypeIndex()]?.SubTypes[GetSubtypeIndex()]?.Name;
    ...
    static class HexStringToPacket
    {
        ...
        {
            if (currentParams.Calibration == null) return input;
